...
public class CadastroSenha extends JFrame implements ActionListener {
        ...
        if (event.getSource() == cadastrarButton) {
            ...
            if (usuario.isEmpty() || senha.isEmpty()) {
                ...
                if (!usuarioExistente) {
                    ...
                    boolean senhaCorreta = verificarSenhaCorreta(usuario, senha);
                    if (senhaCorreta) {
    ...
    private boolean verificarSenhaCorreta(String usuario, String senha) {
        try {
            File arquivo = new File("usuarios.txt");
            if (!arquivo.exists()) {
                return false;
            ...
            while ((linha = reader.readLine()) != null) {
                String[] dadosUsuario = linha.split(";");
                String usuarioExistente = dadosUsuario[0];
                String senhaExistente = dadosUsuario[1];
                if (usuarioExistente.equals(usuario) && senhaExistente.equals(senha)) {
                    reader.close();
                    return true;
                }
            }
